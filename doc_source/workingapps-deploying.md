...
1. Click **Advanced >>** to specify custom JSON\. AWS OpsWorks Stacks adds a set of stack configuration and deployment attributes to the node object\. The `deploy` attributes contain the deployment details and can be used by Deploy recipes to handle installation and configuration\. On Linux stacks, you can use the custom JSON field to override default AWS OpsWorks Stacks settings or pass custom settings to your custom recipes\. For more information about how to use custom JSON, see [Using Custom JSON](workingstacks-json.md)\.
**Note**  
